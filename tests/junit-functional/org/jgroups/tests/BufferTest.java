...
public class BufferTest {
    ...
    public static void testRemoveManyWithWrapping(Buffer<Integer> buf) {
    ...
    public static void testRemoveManyWithFilter(Buffer<Integer> buf) {
        for(int i=1; i <= 10; i++)
            buf.add(i, i);
    ...
    public static void testRemoveManyWithFilterAcceptAll(Buffer<Integer> buf) {
        for(int i=1; i <= 10; i++)
            buf.add(i, i);
    ...
    public void testMassAddition(Buffer<Integer> type) {
        ...
        if(buf instanceof DynamicBuffer)
            ...
            assert buf.capacity() == Util.getNextHigherPowerOfTwo(NUM_ELEMENTS);
    ...
    protected static MessageBatch createMessageBatch(long ... seqnos) {
        ...
        for(long seqno: seqnos) {
            Message m=new EmptyMessage().putHeader(NAKACK3_ID, NakAckHeader.createMessageHeader(seqno));
            mb.add(m);
        }
        return mb;
    }
